...
export default function AddSenderPage() {
        ...
        try {
            const params =
                typeof window !== "undefined"
                    ...
                    : null;
            if (params?.get("resume") === "true" && storage?.addSender?.formData) {
                setFormData(storage.addSender.formData);
                setCurrentStep(storage.addSender.currentStep ?? FormStep.COUNTRY_SELECTION);
            }
        } catch (e) {
            // ignore on server
        }

        if (storage.addSender) {
            const { formData, currentStep } = storage.addSender;
    ...
    useEffect(() => {
        ...
        session.updateSession({ ...storage, addSender: dataToSave });
    ...
    const handleBusinessDetailsSubmit = () => {
        const smileid_business_lastChecked = storage.smileid_business_lastChecked
            ? new Date(storage.smileid_business_lastChecked)
            : null;
        const smileid_business_response = storage.smileid_business_response;

        if (
            smileid_business_response &&
            smileid_business_lastChecked &&
            formData.businessRegistrationNumber ===
            smileid_business_response.company_information?.registration_number
        ) {
            //console.log("Using Local data");
            const diffTime = Math.abs(
                new Date().getTime() - smileid_business_lastChecked.getTime()
            );
            ...
            if (diffDays <= 30) {
                ...
                setFormData((prev) => ({
                    ...prev,
                    businessName:
                        smileid_business_response.company_information?.legal_name ||
                        prev.businessName ||
                        "",
                    businessRegistrationNumber:
                        smileid_business_response.company_information
                            ?.registration_number ||
                        prev.businessRegistrationNumber ||
                        "",
                    dateOfIncorporation:
                        new Date(
                            smileid_business_response.company_information?.registration_date
                        ) ||
    ...
    const fetchBusinessDetails = async () => {
        try {
            ...
            const countryCode = Country.getAllCountries().find(
                (c) => c.name === formData.countryOfIncorporation
            )?.isoCode;
            ...
            const res = await fetch(
                `${Defaults.API_BASE_URL}/sender/verify/business`,
                {
                    method: "POST",
                    headers: {
                        ...Defaults.HEADERS,
            ...
            if (data.status === Status.SUCCESS) {
                ...
                setFormData((prev) => ({
                    ...prev,
                    businessName:
                        parseData.company_information?.legal_name ||
                        prev.businessName ||
                        "",
                    businessRegistrationNumber:
                        parseData.company_information?.registration_number ||
                        prev.businessRegistrationNumber ||
                        "",
                    dateOfIncorporation:
                    ...
                    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4 transform transition-all">
                        ...
                        {submissionState === "success" && (
                            <div className="text-center">
                                ...
                                <p className="text-gray-600 mb-6">
                                    Your sender profile has been successfully created and is ready
                                    to use.
                                </p>
                                <button
                                    onClick={() => {
