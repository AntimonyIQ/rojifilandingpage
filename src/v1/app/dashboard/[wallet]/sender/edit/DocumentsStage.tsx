...
interface BusinessDetailsStageProps {
    ...
    onSubmit?: (submitFunction: () => Promise<boolean>) => void;
}
    ...
    useEffect(() => {
        setUploadedUrls({
            ...
            cacCertOfIncoporation:
                sender.documents?.find((doc) => doc.which === WhichDocument.CERTIFICATE_INCORPORATION)
                    ?.url || null,
            memorandumArticlesOfAssociation:
                sender.documents?.find((doc) => doc.which === WhichDocument.MEMORANDUM_ARTICLES)?.url ||
                null,
            cacStatusReport:
                sender.documents?.find((doc) => doc.which === WhichDocument.INCORPORATION_STATUS)?.url ||
                null,
            proofOfAddress:
                sender.documents?.find((doc) => doc.which === WhichDocument.PROOF_ADDRESS)?.url || null,
            proofOfWealth:
                sender.documents?.find((doc) => doc.which === WhichDocument.PROOF_WEALTH)?.url || null,
            proofOfFunds:
                sender.documents?.find((doc) => doc.which === WhichDocument.PROOF_FUNDS)?.url || null,
        });
    }, []);
    ...
    const FileViewerModal = ({
        ...
        const renderFileContent = () => {
            ...
            if (fileType === "application/pdf") {
                return <iframe src={effectiveUrl} className="w-full h-full border-0" title={fileName} />;
            }
                    ...
                    <DialogHeader className="p-6 pb-2 flex-shrink-0">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <DialogDescription className="text-sm text-gray-600">{fileName}</DialogDescription>
                            </div>
