...
export function KYBVerificationForm() {
      ...
      if (data.status === Status.SUCCESS) {
        ...
        setUploadedUrls({
          cacCertOfIncoporation:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.CERTIFICATE_INCORPORATION
            )?.url || null,
          memorandumArticlesOfAssociation:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.MEMORANDUM_ARTICLES
            )?.url || null,
          cacStatusReport:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.INCORPORATION_STATUS
            )?.url || null,
          proofOfAddress:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.PROOF_ADDRESS
            )?.url || null,
          proofOfWealth:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.PROOF_WEALTH
            )?.url || null,
          proofOfFunds:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.PROOF_FUNDS
            )?.url || null,
          organizationChart:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.ORGANIZATION_CHART
            )?.url || null,
          ownershipChart:
            parseData.sender.documents?.find(
              (d) => d.which === WhichDocument.OWNERSHIP_CHART
            )?.url || null,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (uploadedUrls.cacCertOfIncoporation) {
        documents.push({
          ...
          name:
            formData.cacCertOfIncoporation?.name ||
      ...
      if (uploadedUrls.memorandumArticlesOfAssociation) {
        documents.push({
          ...
          type:
            formData.memorandumArticlesOfAssociation?.type || "application/pdf",
  ...
  const uploadFile = async (file: File, fieldKey: string): Promise<void> => {
    ...
    try {
      ...
      setFieldErrors((prev) => ({
        ...prev,
        [fieldKey]: err.message || "File upload failed",
      }));
    } finally {
        ...
        {fieldUploading[fieldKey] ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-gray-700 truncate">
              {uploadedUrls[fieldKey]
                ? getFilenameFromUrl(uploadedUrls[fieldKey]!)
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-gray-700">
                Selected: {formData[fieldKey]?.name}
              </p>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              >
                ...
                <AlertTitle className="text-sm">
                  Note: Proof of Address requirement
                </AlertTitle>
                <AlertDescription>
                  Kindly ensure the Proof of Address document matches the
              ...
              {renderUploadField("ownershipChart", "Ownership Chart", true)}
