    ...
    try {
      ...
      if (data.status === Status.SUCCESS) {
        ...
        const parseData: IRequestAccess & { sender: ISender } =
          Defaults.PARSE_DATA(data.data, sd.client.privateKey, data.handshake);
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < forms.length; i++) {
      if (!validateForm(forms[i])) {
        setError(
          `Please fill all required fields for ${
            forms[i].firstName || `Person ${i + 1}`
          }`
        );
        return;
      }
  ...
  return (
    <div className="space-y-6 border-b border-gray-200 pb-6 last:border-b-0">
      ...
      {form.isShareholder && (
        <div>
          ...
          <Input
            ...
            type="text"
            className={`h-12 ${
              hasFieldError("shareholderPercentage")
                ? "border-red-500 focus:border-red-500 focus:ring-red-500"
                : ""
            }`}
            inputMode="decimal"
      ...
      <div className="space-y-6">
        ...
        <FileUploadField
          label={`Upload ${
            form.isDirector ? "Director" : "Shareholder"
        ...
        <FileUploadField
          label={`Upload ${
            form.isShareholder ? "Shareholders Registry" : "Directors Registry"
          }`}
          required={true}
          fieldKey={`${index}-${
