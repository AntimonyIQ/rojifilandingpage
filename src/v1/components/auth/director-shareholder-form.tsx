...
import IdentityDocumentTypeOptions, {
    IdentityDocumentType,
    IdentityDocumentTypeEnum,
} from "@/v1/data/identity";
...
export function DirectorShareholderForm() {
    ...
    const loadData = async () => {
        try {
            ...
            const res = await fetch(
                `${Defaults.API_BASE_URL}/requestaccess/approved/sender/${id}`,
                {
                    method: "GET",
                    headers: {
                        ...Defaults.HEADERS,
            ...
            if (data.status === Status.SUCCESS) {
                ...
                const parseData: IRequestAccess & { sender: ISender } =
                    Defaults.PARSE_DATA(data.data, storage.client.privateKey, data.handshake);

                // setCompleted(parseData.completed);

                const directors: Array<IDirectorAndShareholder> =
                    parseData.sender.directors;
                ...
                if (directors && directors.length > 0) {
                    setForms(
                        directors.map((d) => ({
                            ...
                            shareholderPercentage: d.shareholderPercentage
                            ...
                            idType:
                                (d.idType as IdentityDocumentType) ||
                            ...
                            shareholdersSourceOfWealthExplanation:
                                (d as any).shareholdersSourceOfWealthExplanation || [],
                            shareholdersSourceOfWealthEvidence: null,
                            shareholdersSourceOfWealthEvidenceUrl:
                                (d as any).shareholdersSourceOfWealthEvidence?.url || "",
                            directorsRegistry: null,
                            directorsRegistryUrl: (d as any).directorsRegistry?.url || "",
                            shareholdersRegistry: null,
                            shareholdersRegistryUrl:
                                (d as any).shareholdersRegistry?.url || "",
                        }))
                    );
                } else {
                    setForms([createNewForm()]);
    ...
    const handleFileUpload = async (
        ...
        try {
            ...
            if (data.status === Status.SUCCESS) {
                if (!data.handshake)
                    throw new Error("Unable to process upload response");
            ...
            setFieldErrors((prev) => ({
                ...prev,
                [uploadKey]: err.message || "File upload failed",
            }));
        } finally {
    ...
    const validateForm = (form: DirectorShareholderFormData): boolean => {
        const hasIdDocument =
            !!form.idDocument ||
            (!!form.idDocumentUrl && form.idDocumentUrl.trim() !== "");
        const hasProofOfAddress =
            !!form.proofOfAddress ||
            (!!form.proofOfAddressUrl && form.proofOfAddressUrl.trim() !== "");
        const hasShareholderPercentage =
            !form.isShareholder || form.shareholderPercentage.trim() !== "";
        const hasSourceOfWealthExplanation =
            form.shareholdersSourceOfWealthExplanation.length > 0;
        const hasSourceOfWealthEvidence =
            !!form.shareholdersSourceOfWealthEvidence ||
            (!!form.shareholdersSourceOfWealthEvidenceUrl &&
                form.shareholdersSourceOfWealthEvidenceUrl.trim() !== "");
        ...
        const hasRegistry = form.isShareholder
            ? !!form.shareholdersRegistry ||
            (!!form.shareholdersRegistryUrl &&
                form.shareholdersRegistryUrl.trim() !== "")
            : !!form.directorsRegistry ||
            (!!form.directorsRegistryUrl &&
                form.directorsRegistryUrl.trim() !== "");
    ...
    // console.log(forms);
        ...
        for (let i = 0; i < forms.length; i++) {
            if (!validateForm(forms[i])) {
                setError(
                    `Please fill all required fields for ${forms[i].firstName || `Person ${i + 1}`
                    }`
                );
                return;
            }
        ...
        try {
            ...
            const submitData = forms.map((form) => ({
                ...
                shareholderPercentage: form.isShareholder
                    ? Number(form.shareholderPercentage)
                    : undefined,
                ...
                shareholdersSourceOfWealthExplanation:
                    form.shareholdersSourceOfWealthExplanation,
                shareholdersSourceOfWealthEvidence:
                    form.shareholdersSourceOfWealthEvidenceUrl,
                directorsRegistry: form.isShareholder
                    ? undefined
                    : form.directorsRegistryUrl,
                shareholdersRegistry: form.isShareholder
                    ? form.shareholdersRegistryUrl
                    : undefined,
            }));
            ...
            if (data.status === Status.SUCCESS) {
                toast.success(
                    "Directors/Shareholders information submitted successfully"
                );
            ...
            <div className="fixed inset-0 bg-white flex items-center justify-center">
                <div className="text-center max-w-lg px-6">
                    ...
                    <p className="mt-2 text-gray-600">
                        You currently don't have access to this page. Please request access
                        to continue.
                    </p>
                    <div className="mt-6">
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-white">
                <div className="p-6 max-w-md mx-auto text-center">
                    ...
                    <p className="text-gray-600 mb-4">
                        You have successfully submitted your documents. They are under
                        review — you will be notified once the review is complete.
                    </p>
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center py-8 px-6">
                        ...
                        >
                            ...
                            <div className="absolute -inset-4">
                                <div
                                    className="absolute top-0 left-1/2 w-2 h-2 bg-green-400 rounded-full animate-ping"
                                    style={{ animationDelay: "0ms" }}
                                />
                                <div
                                    className="absolute bottom-0 right-0 w-1.5 h-1.5 bg-green-500 rounded-full animate-ping"
                                    style={{ animationDelay: "200ms" }}
                                />
                                <div
                                    className="absolute top-1/2 left-0 w-1 h-1 bg-green-300 rounded-full animate-ping"
                        ...
                        >
                            ...
                            <DialogDescription className="text-lg text-gray-600 leading-relaxed max-w-md">
                                Your KYC information has been received and it's currently under
                                review.
                                <br />
                                <span className="font-medium text-gray-700">
                                    We'll notify you via email/dashboard once the review is
                                    completed.
                                </span>
                            </DialogDescription>
                        </motion.div>
...
function DirectorShareholderFormCard({
    ...
    const handleRoleChange = (
        ...
        onFormChange(
            index,
            roleType === "director" ? "isDirector" : "isShareholder",
            checked
        );

        // Update role display
        const newIsDirector = roleType === "director" ? checked : form.isDirector;
        const newIsShareholder =
            roleType === "shareholder" ? checked : form.isShareholder;
    ...
    return (
        <div className="space-y-6 border-b border-gray-200 pb-6 last:border-b-0">
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                        <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                                handleRoleChange("director", checked as boolean)
                            }
                        />
                    ...
                    <div className="flex items-center space-x-2">
                        <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                                handleRoleChange("shareholder", checked as boolean)
                            }
                        />
            ...
            {form.isShareholder && (
                <div>
                    ...
                    <Input
                        ...
                        onBlur={(e) =>
                            handleFieldBlur("shareholderPercentage", e.target.value)
                        }
                        placeholder=""
                    />
            ...
            <div>
                ...
                <Input
                    id={`email-${index}`}
                    className={`h-12 ${hasFieldError("email")
                        ? "border-red-500 focus:border-red-500 focus:ring-red-500"
                        : ""
                        }`}
                    value={form.email}
                            ...
                            >
                                <img
                                    src={`https://flagcdn.com/w320/${countries
                                        .find(
                                            (country) => country.name === form.selectedCountryCode
                                        )
                                        ...
                                        {countries.map((country: any, countryIndex: number) => (
                                            ...
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        form.selectedCountryCode === country.name
                                                            ? "opacity-100"
                                                            : "opacity-0"
                                                    )}
                                                />
                                    ...
                                    {countries.map((country: any) => (
                                        ...
                                        >
                                            <Check
                                                className={cn(
                                                    "mr-2 h-4 w-4",
                                                    form.issuedCountry === country.name
                                                        ? "opacity-100"
                                                        : "opacity-0"
                                                )}
                                            />
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <Input
                        id={`city-${index}`}
                        className={`h-12 ${hasFieldError("city")
                            ? "border-red-500 focus:border-red-500 focus:ring-red-500"
                            : ""
                            }`}
                        value={form.city}
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <Input
                        id={`state-${index}`}
                        className={`h-12 ${hasFieldError("state")
                            ? "border-red-500 focus:border-red-500 focus:ring-red-500"
                            : ""
                            }`}
                        value={form.state}
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900">
                    {form.isDirector ? "Director" : "Shareholder"} Source of Wealth{" "}
                    <span className="text-red-500">*</span>
                </h3>
                <div className="space-y-3">
                    {sourceOfWealthOptions.map((source) => (
                        <div key={source.value} className="flex items-center space-x-2">
                            <Checkbox
                                ...
                                checked={form.shareholdersSourceOfWealthExplanation.includes(
                                    source.value
                                )}
                                onCheckedChange={(checked) => {
                                    const currentArray =
                                    ...
                                    if (checked) {
                                        onFormChange(
                                            index,
                                            "shareholdersSourceOfWealthExplanation",
                                            [...currentArray, source.value]
                                        );
                                    } else {
                                        onFormChange(
                                            index,
                                            "shareholdersSourceOfWealthExplanation",
                                            currentArray.filter((item) => item !== source.value)
                                        );
                                    }
                                }}
                            />
            ...
            {form.shareholdersSourceOfWealthExplanation.length > 0 && (
                ...
                >
                    ...
                    <AlertDescription>
                        We would require documents as proof of your selected source(s) of
                        wealth for verification.
                    </AlertDescription>
                </Alert>
            )}
            ...
            <div className="space-y-6">
                ...
                />
                <FileUploadField
                    label={`Upload ${form.isDirector ? "Director" : "Shareholder"
                    ...
                    uploading={
                        uploadingFiles[`${index}-shareholdersSourceOfWealthEvidence`]
                    }
                    error={fieldErrors[`${index}-shareholdersSourceOfWealthEvidence`]}
                    onFileSelect={(file) =>
                        onFileUpload(file, index, "shareholdersSourceOfWealthEvidence")
                    }
                    onFileRemove={() =>
                        handleFileRemove("shareholdersSourceOfWealthEvidence")
                    }
                />
                <FileUploadField
                    ...
                    onFileSelect={(file) =>
                        onFileUpload(
                            file,
                            index,
                            form.isShareholder ? "shareholdersRegistry" : "directorsRegistry"
                        )
                    }
                    onFileRemove={() =>
                        handleFileRemove(
