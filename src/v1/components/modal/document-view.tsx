...
export default function DocumentViewerModal({
    ...
    // Detect if device is mobile or tablet (iPad, iPhone, Android tablets, etc.)
    useEffect(() => {
        ...
        setIsMobileOrTablet(isMobile || isTablet || (isTouchDevice && window.innerWidth <= 1024));
    }, []);
    ...
    const renderPdfViewer = () => {
        // Use direct link for PC/Desktop (works better, shows all pages)
        // Use Google Docs Viewer for iPad/Mobile (better compatibility but has loading issues)
        const directPdfUrl = `${documentUrl}#toolbar=1&navpanes=1&scrollbar=1&page=1&view=FitH`;
        const googleViewerUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(documentUrl)}&embedded=true`;
        
        const pdfUrl = isMobileOrTablet ? googleViewerUrl : directPdfUrl;
            ...
            >
                ...
                {error ? (
                    ...
                    <iframe
                        ...
                        title={documentTitle}
                        onLoad={() => {
                            console.log(`PDF loaded using ${isMobileOrTablet ? 'Google Viewer (Mobile/Tablet)' : 'Direct Link (Desktop)'}`);
                            setIsLoading(false);
                        }}
                        onError={() => {
