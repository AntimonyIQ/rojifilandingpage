...
export function PayAgainModal({
  ...
  useEffect(() => {
    if (storage) {
      ...
      const matchingWallet: IWallet | undefined = storage.wallets.find(
        (w) => w.currency === Fiat.USD
      );
  ...
  useEffect(() => {
    ...
    const rawPhone = transaction.phoneNumber
      ? String(transaction.phoneNumber).replace(/\D/g, "")
      : "";
    const phoneCodeDigits = transaction.phoneCode
      ? String(transaction.phoneCode).replace(/\D/g, "")
      : "";
    const phoneNumber =
      phoneCodeDigits && rawPhone.startsWith(phoneCodeDigits)
    ...
    const payAgainData: IPayment = {
      ...
      senderName: storage.sender
        ? storage.sender.businessName
        ...
        setFormdata(
          (prev) =>
            ({
              ...
              beneficiaryCurrency:
                countries.find((c) => c.isoCode === parseData.country)
                  ?.currency || "",
              paymentRail:
  ...
  const validateForm = (): { isValid: boolean; errors: string[] } => {
    ...
    if (formdata.beneficiaryAmount) {
      if (!isValidAmount(formdata.beneficiaryAmount)) {
        errors.push(
          "Beneficiary amount must be a valid number with up to 2 decimal places"
        );
      }
    }
      ...
      const payload = {
        ...
        bankData: {
          ...
          recipientInfo: {
            ...
            recipientCountry:
              findCountryByName(formdata.beneficiaryCountry)?.isoCode || "", // beneficiaryCountry
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full sm:w-[90%] md:w-[70%] lg:w-[50%] h-[95dvh] bg-white rounded-lg shadow-xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {uploadError &&
                uploadError.includes("Please fix the following:") && (
              ...
              <div className="space-y-4">
                <div className="relative mb-5">
                  <div
                    className={`group flex items-center gap-4 p-4 border-2 rounded-xl transition-all duration-200 ${
                      formdata?.swiftCode
                      ...
                      {formdata?.swiftCode ? (
                        ...
                        <div className="text-gray-500">
                          <div className="font-medium text-gray-700">
                            Invalid SWIFT code selected
                          </div>
                          <div className="text-sm text-gray-500 mt-1">
                            Click Select to choose your beneficiary bank
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {formdata && !loading && (
                <>
                  ...
                  {formdata.senderCurrency === Fiat.USD &&
                    (!formdata.swiftCode || formdata.swiftCode.length <= 7) && (
                      <div className="text-center py-12">
                        <p className="text-gray-600">
                          Please provide SWIFT/BIC code to continue with USD
                          payment.
                        </p>
                      </div>
                    )}
