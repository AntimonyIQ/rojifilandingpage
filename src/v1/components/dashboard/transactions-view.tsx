...
import { EyeOff, Wallet, Search, X } from "lucide-react";
...
export function TransactionsView({}: TransactionsViewProps) {
  ...
  // UI state
  const [_activeFiltersCount, setActiveFiltersCount] = useState(0);
  ...
  const currencies: Array<ICurrency> = [
    {
      name: "All",
      icon: "https://img.icons8.com/color/50/worldwide-location.png",
    },
    ...
    {
      name: "EUR",
      icon: "https://img.icons8.com/fluency/48/euro-pound-exchange.png",
    },
    {
      name: "GBP",
      icon: "https://img.icons8.com/color/50/british-pound--v1.png",
    },
  ];
  ...
  //     if (storage.transactionsTableData[statusFilter]) {
  //       setTransactions(storage.transactionsTableData[statusFilter]);
  //     }
  //     setCounts(storage.transactionCounts);
  //   }, []);

  //   // Handle status filter changes - use cached data if available
  //   useEffect(() => {
  //     if (
  //       storage.transactionsTableData[statusFilter] &&
  //       storage.transactionsTableData[statusFilter].length > 0
  //     ) {
  //       setTransactions(storage.transactionsTableData[statusFilter]);
  //     } else {
  //       setTransactions([]);
  ...
  const fetchCounts = async () => {
    try {
      ...
      if (data.status === Status.SUCCESS) {
        ...
        const parseData: { [key in TransactionStatus]: number } =
          Defaults.PARSE_DATA(
            data.data,
            storage.client.privateKey,
            data.handshake
        ...
        setCounts(parseData);
        session.updateSession({ ...storage, transactionCounts: parseData });
      }
    } catch (error: any) {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-5 justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            View and manage all your transaction history across different
            currencies
          </p>
        </div>
      ...
      <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-1 p-1 bg-gray-100 rounded-lg">
              {statusTabs.map((status) => (
                <button
                  ...
                  className={`px-3 py-2 text-sm font-medium capitalize rounded-md transition-colors whitespace-nowrap ${
                    statusFilter === status
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="relative flex-1 lg:max-w-md">
            ...
            <Input
              placeholder="Search transactions by Beneficiary..."
              value={searchQuery}
            ...
            <CardContent className="p-0">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-gray-200 gap-4">
                <div className="text-sm text-gray-700">
                  ...
                  of {pagination.total} entries ({pagination.limit} per page)
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
