...
export function DepositView() {
    ...
    const [selectedCurrency, setSelectedCurrency] = useState<IWallet | null>(
        null
    );
    const [usdToken, setUsdToken] = useState("USDC");
    const [network, setNetwork] = useState("");
    const [_selectedDepositOption, setSelectedDepositOption] =
        useState<any>(null);
    ...
    useEffect(() => {
        ...
        if (sel && Array.isArray(sel.deposit) && sel.deposit.length > 0) {
            if (sel.currency !== Fiat.NGN) {
                ...
                const usdcEthOptions = sel.deposit.filter(
                    (d) => d.currency === "USDC" && d.network === "ETH"
                );
                ...
                const first = sel.deposit[0] as any;
                const defaultVal =
                    first.institution ?? first.currency ?? first.network ?? usdToken;
        ...
        try {
            ...
            if (data.status === Status.SUCCESS) {
                ...
                if (userdata.status === Status.SUCCESS) {
                    ...
                    setSelectedCurrency(
                        parseData.wallets.find((w) => w.currency === wallet) || null
                    );
    ...
    const getDepositDetails = () => {
        if (selectedCurrency?.currency === Fiat.NGN) {
            const bankOption = selectedCurrency.deposit.find(
                (d) => d.institution === usdToken
            );
            return {
                                        ...
                                        <div>
                                            ...
                                            <p className="text-emerald-100 text-sm">
                                                {selectedCurrency?.currency === Fiat.NGN
                                                    ? "Bank Transfer"
                                                    : "Dollar Deposit to your account"}
                                            </p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            */}
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="flex items-center gap-3 p-4 bg-gray-50 border-2 border-gray-200 rounded-xl hover:bg-gray-100 transition-colors">
                                                        ...
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            onClick={() =>
                                                                handleCopy(depositDetails.accountName!)
                                                            }
                                                            className="text-emerald-600 hover:text-emerald-700 hover:bg-emerald-100"
                                                        >
                                                            <Copy className="h-4 w-4" />
                                            ...
                                            <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                                                <LoaderIcon
                                                    className={`h-4 w-4 ${isCheckingDeposits ? "animate-spin" : ""
                                                        }`}
                                                />
                                                        ...
                                                        <div className="flex items-start gap-3">
                                                            ...
                                                            <div className="text-sm text-amber-800">
                                                                ...
                                                                <p>
                                                                    ...
                                                                    network. Wrong network transfers cannot be
                                                                    recovered.
                                                                </p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="text-center space-y-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-gray-600 mb-4">
                                                                    No {usdToken} wallet found on {network} network.
                                                                    Create one to start receiving deposits.
                                                                </p>
                                                            </div>
                                                            <Button
                                                    ...
                                                    <div className="flex flex-col items-center justify-center p-8 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl">
                                                        <div className="text-center space-y-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-gray-500">
                                                                    Please select both cryptocurrency and network to
                                                                    view or create your wallet.
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </div>
                    ...
                    <DialogHeader className="text-center space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <DialogDescription className="text-gray-600">
                                Your {depositResponse?.currency || selectedCurrency?.currency}{" "}
                                wallet has been credited with {selectedCurrency?.symbol}
                                {depositResponse?.amount?.toLocaleString() || "0"}.
                            </DialogDescription>
                        </div>
                    </DialogHeader>
