...
export function DashboardOverview() {
  ...
  const [wallets, setWallets] = useState<IWallet[]>(() => {
    return session.getUserData()?.wallets || [];
  });
  const [selectedCurrency, setSelectedCurrency] = useState<Fiat>(Fiat.NGN);
  const [withdrawalActivated, setWithdrawalActivated] =
    useState<boolean>(false);
  const [swapActivated, setSwapActivated] = useState<boolean>(false);
  const [depositActivated, setDepositActivated] = useState<boolean>(false);
  ...
  const [allTransactions, setAllTransactions] = useState<Array<ITransaction>>(
    []
  );
  const [loadingAllTx, setLoadingAllTx] = useState<boolean>(false);
  const [loadingRecentTx, setLoadingRecentTx] = useState<boolean>(false);
  ...
  const walletDetails = useCallback(async () => {
    try {
      ...
      if (data.status === Status.SUCCESS) {
        if (!data.handshake)
          throw new Error("Unable to process response right now.");
        ...
        session.updateSession({ ...storage, wallets: parseData.wallets });
  ...
  useEffect(() => {
    ...
    //     totalbeneficiary:
    //       txStat.totalbeneficiary ?? defaultTxStat.totalbeneficiary,
    ...
    //   const activeWallet: IWallet | undefined = (storage.wallets || []).find(
  ...
  const fetchTransactionStatistics = async () => {
    ...
    try {
      ...
      if (data.status === Status.SUCCESS) {
        ...
        const parseData: ITransactionsStat = Defaults.PARSE_DATA(
          data.data,
          storage.client.privateKey,
          data.handshake
        );
  ...
  const connectWebSocketRates = () => {
    ...
    const ws = new WebSocket(
      `${Defaults.WSS_BASE_URL}/rates?handshake=${storage.client.publicKey}&authorization=${storage.authorization}`
    );
          ...
          <div className="flex items-center justify-between mb-4 pt-4">
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-500">
                An overview of your payment activities
              </p>
            ...
            <div className="flex items-center gap-3">
              <Select
                value={chartFilter}
                onValueChange={(value) =>
                  setChartFilter(value as ChartFilterOptions)
                }
              >
              ...
              <button
                ...
                className="text-sm text-gray-500 hidden md:flex items-center gap-1"
              >
  ...
  const getTxDetails = (
    ...
    switch (type) {
      case TransactionType.DEPOSIT:
        return `Wallet funded successfully with ${amount.toLocaleString(
          undefined,
          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
        )} ${currency}`;
      case TransactionType.WITHDRAWAL:
        return `Wallet debited successfully with ${amount.toLocaleString(
          undefined,
          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
        )} ${currency}`;
      case TransactionType.TRANSFER:
        return `Transfer of ${amount.toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })} ${currency}`;
      case TransactionType.SWAP:
        return `${currency} swapped to ${toCurrency} successfully with ${amount.toLocaleString(
          undefined,
          { minimumFractionDigits: 2, maximumFractionDigits: 2 }
        )} ${currency}`;
      default:
  ...
  const formatDate = (input?: string | Date) => {
    ...
    const time = `${String(hour12).padStart(
      2,
      "0"
  ...
  const fetchAllTransactions = async () => {
    try {
      ...
      const res = await fetch(
        `${Defaults.API_BASE_URL}/transaction/all?primarySender=${storage.sender._id}`,
        {
          method: "GET",
          headers: {
            ...Defaults.HEADERS,
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-6">
          <div className="flex flex-col items-start gap-5 w-full">
                ...
                <div className="flex flex-wrap gap-1 p-1 bg-gray-100 rounded-lg">
                  {sortedWallets.map((tab) => (
                    <button
                      ...
                      className={`px-3 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap flex flex-row items-center gap-1 ${
                        selectedCurrency === tab.currency
            ...
            {["EUR", "GBP"].includes(selectedCurrency) ? (
              <>
                {activeWallet &&
                activeWallet.requested &&
                activeWallet.requested.find(
                  (r) =>
                    r.currency === selectedCurrency && r.status === "pending"
                ) ? (
                  <div className="w-full mt-40 flex flex-col items-center justify-center">
                    <div className="relative flex items-center justify-center w-20 h-20">
                      ...
                      <Wallet
                        size={28}
                        className="relative z-10 text-yellow-900"
                      />
                    </div>
                    <div className="flex flex-col items-center text-center justify-center gap-2 mt-3">
                      <h2 className="font-bold">
                        {selectedCurrency} Wallet Request Pending
                      </h2>
                      <p>
                        Your request to activate a {selectedCurrency} wallet is
                        pending approval.
                        <br />
                  ...
                  <div className="w-full mt-40 flex flex-col items-center justify-center">
                    <div className="relative flex items-center justify-center w-20 h-20">
                      ...
                      <Wallet
                        size={28}
                        className="relative z-10 text-blue-900"
                      />
                    </div>
                    ...
                    <div className="flex flex-col items-center text-center justify-center gap-2 mt-3">
                      ...
                      <p>
                        This feature isnt enabled for you, however you can
                        initiate a {selectedCurrency} transfer from <br />
                        <span className="text-blue-500 font-medium capitalize">
                          "create payment"
                        </span>{" "}
                        then choose {selectedCurrency}.
                      </p>
              ...
              <>
                    ...
                    <div className="flex flex-row items-center justify-start gap-5 w-full py-3 overflow-x-auto scroll-smooth !scrollbar-hide">
                        ...
                        <div className="flex-shrink-0 px-4 py-3 rounded-lg bg-slate-300 w-[300px] h-[150px] gap-4 flex flex-col items-start justify-between">
                          <div className="flex flex-col items-start justify-start">
                            <div className="text-2xl">
                              {hideBalances
                                ? "•••••"
                                : (txstat?.pending || 0).toLocaleString(
                                    "en-US"
                                  )}
                            </div>
                          ...
                          <Button variant="outline">
                            <a
                              href={`/dashboard/${selectedCurrency}/transactions?status=rejected`}
                      ...
                      {activeWallet.currency !== Fiat.NGN && (
                        <div className="flex-shrink-0 px-4 py-3 rounded-lg bg-[#ffe8c3] w-[300px] h-[150px] gap-4 flex flex-col items-start justify-between">
                          ...
                          <Button variant="outline">
                            <a
                              href={`/dashboard/${selectedCurrency}/beneficiary`}
                              className="text-xs uppercase"
                            >
                              View Recipient
                            </a>
                ...
                <div className="flex flex-row items-center justify-start gap-2">
                  <Button
                    ...
                    onClick={(): void => {
                      if (
                        ...
                        setDepositActivated(true);
                      } else {
                        navigate(`/dashboard/${selectedCurrency}/deposit`);
                      }
                    }}
                  >
                    <a
                      // href={`/dashboard/${selectedCurrency}/deposit`} hover:bg-slate-50 capitalize border rounded-lg px-5 bg-white
                      className="flex flex-row items-center justify-center text-center py-2 gap-2 "
                    >
                  ...
                  {selectedCurrency === Fiat.NGN && (
                    ...
                    >
                      <a
                        //href={`/dashboard/${selectedCurrency}/swap`}
                    ...
                    <div className="divide-y divide-gray-50">
                      {liveRates
                        ...
                        .map((rate, idx) => {
                          ...
                          const shouldReverse =
                            (rate.from === "EUR" || rate.from === "GBP") &&
                            rate.to === "USD";
                          const displayFrom = shouldReverse
                            ? rate.to
                            : rate.from;
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div className="flex items-center -space-x-2">
                                  <img
                                    src={
                                      displayFromIcon || "/placeholder-icon.png"
                                    }
                                    alt={displayFrom}
                                  ...
                                  <img
                                    src={
                                      displayToIcon || "/placeholder-icon.png"
                                    }
                                    alt={displayTo}
                                ...
                                <div className="p-4 space-y-4">
                                  ...
                                  <div className="border-b pb-3">
                                    ...
                                    <div className="font-medium">
                                      {selectedTx
                                        ? `${selectedTx.fromCurrency} ${Number(
                                            selectedTx.amount
                                          ).toLocaleString()}`
                                        : ""}
                                    </div>
      ...
      <Dialog open={withdrawalActivated} onOpenChange={setWithdrawalActivated}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedCurrency} Withdrawal is currently disabled. Click on{" "}
            ...
            <Button
              ...
              onClick={() => {
                const subject = encodeURIComponent(
                  `${selectedCurrency} Withdrawal Request`
                );
      ...
      <Dialog open={swapActivated} onOpenChange={setSwapActivated}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Activate {selectedCurrency} Swap</DialogTitle>
            <DialogDescription>
              {selectedCurrency} Swap is currently disabled. Click on{" "}
              <span className="text-blue-500">"Contact Support"</span> to enable
              Swap.
            </DialogDescription>
          </DialogHeader>
      ...
      {/* deposit */}
      <Dialog open={depositActivated} onOpenChange={setDepositActivated}>
        <DialogContent>
          <DialogHeader>
      ...
      >
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete history of your deposit and swap transactions
            </DialogDescription>
          </DialogHeader>
